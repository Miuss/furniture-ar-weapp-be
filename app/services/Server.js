...
export const queryServerTotalData = async (serverId) => {
  ...
  const sevenDayOnlinePlayer = result1.length > 0 ? parseFloat(result1[0].online) : 0
  ...
  const maxDayOnlinePlayer = result2.length > 0 ? result2[0].online : 0

  // 历史最高
  const [result3] = await sequelize.query(`SELECT max(online) as online FROM server_data WHERE sid = ${serverId}`)                                                                       
  const maxOnlinePlayer = result3.length > 0 ? result3[0].online : 0
